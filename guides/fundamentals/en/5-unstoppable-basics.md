...
Below we will go through some important points to keep in mind when using non-custodial wallet apps. We are going to use SendChain wallet app to illustrate these important aspects. 
...
- The 'Restore Wallet' option restores an existing wallet that might have been created earlier in SendChain or some other standard compliant wallet. 
...
SendChain aims to provide a comprehensive overview for various cryptocurrencies in simple terms.
...
Some wallets including SendChain enables creation of unlimited number of wallets within a single app. 
...
- **Balance and Transactions**
    ...
    To address this, some wallets like SendChain support restoring directly from the blockchain network itself rather than via a third-party server. This ensures that wallet provider has no clue about balance or transactions of the user.
...
- **Blockchain Privacy**
    ...
    Certain non-custodial wallets may have additional features which increase the privacy on that specific blockchain. For instance, SendChain wallet app integrates features which make it significantly more difficult linking different Bitcoin transactions to a single entity. 
